...
Greenfield: No existing backend/UI in Projects for this app; particle-camera is out of scope.
...
Modular boundaries, guards for RBAC, first-class DI, easy OpenAPI
...
JSON for flexible NLP metadata; full-text search optional (tsvector)
...
Rate limits, session store, async voice pipeline
...
OAuth2 (Google/Microsoft) + JWT (short-lived access + rotating refresh in httpOnly cookie or secure storage on mobile)



Industry standard; NestJS @nestjs/passport
...
S3-compatible (AWS S3, Azure Blob, MinIO)



Presigned URLs for direct browser upload; virus scan in worker (optional ClamAV)
...
Socket.io or native WS in NestJS



Session updates, notification when voice job completes
...
React + TanStack Query + Zustand/Context; responsive CSS (Tailwind or MUI)



Fast iteration; role-based route splits
...
Filter teachers by subject (from NLP) and availability/price bounds if extracted.



Score: weighted sum of rating, tag overlap with NLP keywords, specialization match, optional embedding similarity between problem text and teacher bio/tags.



Return top N with explainability (short reason per teacher).
...
Shell: Auth pages, layout by role (/teacher/*, /student/*), protected routes.



Teacher dashboard: sessions list/calendar, student progress views, document manager, profile/settings.



Student dashboard: sessions, progress charts (e.g. Recharts), saved teachers, documents, profile.



Discovery: filterable teacher grid + detail page.



Voice UX: MediaRecorder API, upload progress, job status polling or WebSocket, results list with match reasons.



Responsive: mobile-first breakpoints; touch-friendly controls for recording.
...
Project bootstrap: NestJS modules — Auth, Users, Teachers, Students, Sessions, Documents, Voice, Notifications (optional).



Persistence: migrations (Prisma/TypeORM); seed subjects/tags.



Storage service: abstraction over S3 for presigned URLs.



Workers: Bull processors for processVoiceQuery.



Integration: STT + NLP clients with timeouts, retries, structured errors.



Observability: structured logging (Pino), request IDs, health checks /health/live and /health/ready.
...
Diagrams + module boundaries (this plan)
...
Migrations + ERD from Prisma/TypeORM
...
Sequence + worker + storage contract
...
JWT/OAuth, RBAC, storage hardening
...
Docker + env matrix + CI/CD outline
...
MVP: Auth, roles, teacher/student profiles, teacher search (no voice), sessions, documents, basic dashboards.



V2: Voice upload → STT → NLP → matching; WebSocket notifications.



V3: Embeddings-based ranking, advanced analytics, native mobile app if needed (API already supports PWA).
